...
########################################################### avaliar o estado é verificar se está na meta
...
def celulas_vazias(estado):
    # incialização da lista de células vazias , como uma list vazia
    lista_celula_vazias = []
...
########################################################### celulas vazia vao ser os next states
...
############################################################# aplicar o movimento é a função de try next positon
...
def turno_bot(escolha_bot, escolha_humana):
    ...
    render(tabuleiro, escolha_bot, escolha_humana)  ########################################################## render mudar para o show map

    if profundidade == 9:      ##########################################################  escolhe sempre um dos 9 movimentos
        x = rd.choice([0, 1, 2])
        y = rd.choice([0, 1, 2])
    else:
    ...
    aplica_movimento(x, y, BOT)    ####################################### aplicar movimento é preencher a posição com C, pode ir para o mesmo sitio como no jogo?
    time.sleep(2)
...
while len(celulas_vazias(tabuleiro)) > 0 and \
        not (verifica_vitoria(tabuleiro, HUMANO) or verifica_vitoria(tabuleiro, BOT)):
    if primeiro == 'N':
